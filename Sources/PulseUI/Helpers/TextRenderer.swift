...
final class TextRenderer {
    ...
    func renderConnectionSummary(_ task: NetworkTaskEntity, store: LoggerStore) {
        // Header with status
        let statusTitle = task.connectionDisplayTitle
        let statusColor: UXColor = task.isConnectionPaused
            ? .systemPink
            : task.connectionState == .complete ? ConnectionColors.complete : ConnectionColors.connected
